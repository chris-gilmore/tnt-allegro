...
void Font_Deinit(Font *p_font) {
  al_destroy_bitmap(p_font->image);
  p_font->image = NULL;
}
...
void Font_Init46Char(Font *p_font, u32 img_id) {
  ...
  if (img_id != 0) {
    ...
    p_font->image = al_load_bitmap_flags(config_setting_get_string_elem(images, img_id), ALLEGRO_NO_PREMULTIPLIED_ALPHA);
    if (p_font->image == NULL) {
      return;
    }

    p_font->width = al_get_bitmap_width(p_font->image);
    p_font->height = al_get_bitmap_height(p_font->image) / 46;

    locked = al_lock_bitmap(p_font->image, ALLEGRO_PIXEL_FORMAT_RGBA_4444, ALLEGRO_LOCK_READONLY);
    ...
    al_unlock_bitmap(p_font->image);

    al_destroy_bitmap(p_font->image);
    p_font->image = al_load_bitmap(config_setting_get_string_elem(images, img_id));
  }
}
...
void Font_Init51Char(Font *p_font, u32 img_id) {
  ...
  if (img_id != 0) {
    ...
    p_font->width = al_get_bitmap_width(p_font->image);
    p_font->height = al_get_bitmap_height(p_font->image) / 51;
